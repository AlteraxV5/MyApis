...
const { getApiKeys, validateApiKey, checkRateLimit, getVisitorData, updateVisitorData, hashKey, getCurrentWindow } = require('./src/supabaseHelper');
...
const checkApiKey = async (req, res, next) => {
    ...
    if (req.path === '/api/key-info') return next();
...
app.get('/api/key-info', async (req, res) => {
    ...
    try {
        ...
        const used = rateData?.hit_count || 0;
        ...
        res.json({
            ...
            info: {
                role: keyData.service_name,
                limit: isUnlimited ? '∞' : limitPerHour,
                used: isUnlimited ? 0 : used,
                remaining: isUnlimited ? '∞' : Math.max(0, limitPerHour - used),
                unlimited: isUnlimited,
                reset_in: `${60 - new Date().getUTCMinutes()} menit`
            }
        });
    } catch (err) {
        console.error('[key-info] Error:', err.message);
