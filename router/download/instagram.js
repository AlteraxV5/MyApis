...
class InstaShadow {
  constructor() {
    ...
    this.keyMap = {
      ...
      id: "media_id", shortcode: "shortcode", om: "other_media", fn: "full_name",
      pp: "profile_pic", hpp: "hd_profile_pic", frc: "follower_count",
      fgc: "following_count", mc: "media_count", b: "biography"
    }
  }
  ...
  _parse(url) {
    ...
    } catch (e) { return { ep: "posts", pl: { _u: url ?? "" } } }
  }
  ...
  async _sign(pl, t) {
    ...
    } catch (e) { return { ...pl || {}, _s: "", _s1: Date.now() } }
  }
  ...
  async download({ url, ...rest }) {
    ...
    return { status: response?.status === 200, result: this._proc(response?.data) }
  }
}
...
module.exports = async function (req, res) {
  ...
  try {
    ...
    const { status, result } = await api.download({ url })

    if (!status) return res.status(500).json({ status: false, message: "Gagal mengambil data dari Instagram." })
