...
function createSigner(code) {
  ...
  reals.atob = global.atob; reals.btoa = global.btoa;
...
async function igram(input) {
  const t = detect(input);
  ...
  if (r.captcha) return { status: false, type: t.kind, error: "captcha unsolved after " + r.attempts + " attempts" };
  if (r.status !== 200 || (r.data && r.data.success === false)) return { status: false, type: t.kind, error: (r.data && (r.data.message || r.data.info)) || ("http " + r.status) };

  const base = { status: true, type: t.kind, captchaAttempts: r.attempts };
  if (t.kind === "media") return { ...base, ...mediaResult(r.data) };
  if (t.kind === "story" || t.kind === "stories") return { ...base, username: t.username || "", media: parseStories(r.data) };
  return { ...base, profile: parseProfile(r.data) };
}
