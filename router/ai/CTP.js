...
async function geminiChat(message, options = {}) {
    ...
    return {
        ...
        sessionId: ok ? pack({
            ...
            customPrompt: options.customPrompt || sess?.customPrompt || ""
...
module.exports = async function (req, res) {
    ...
    const customPrompt = req.query?.customPrompt || req.body?.customPrompt || ""

    if (!q) return res.status(400).json({ status: false, message: "Parameter 'q' diperlukan." })
    if (!customPrompt) return res.status(400).json({ status: false, message: "Parameter 'customPrompt' diperlukan." })

    const sess = sessionId ? unpack(sessionId) : null
    const activePrompt = sess ? (sess.customPrompt || "") : customPrompt

    const message = activePrompt ? `${activePrompt}\n\n${q}` : q

    try {
        const result = await geminiChat(message, { sessionId, customPrompt: activePrompt })

        if (!result.response) return res.status(500).json({ status: false, message: "Gemini tidak memberikan respon." })
